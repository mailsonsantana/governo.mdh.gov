...
class IComposicaoTile(IListTile):
    """Display a composicao of items."""
...
@implementer(IComposicaoTile)
class ComposicaoTile(ListTile):
    """Display a composicao of items."""

    is_configurable = False
    is_droppable = True
    is_editable = True
    short_name = _(u'msg_short_name_composicao', default=u'Composicao')
    ...
    def results(self):
        ...
        for i, item in enumerate(super(ComposicaoTile, self).results()):
            page.append(item)
            if (i + 1) % 3 == 0:
                yield page
                page = []
    ...
    def get_image(self,item):
        if item:
            ...
            scale = scales.scale('image', scale='composicao_foto_view')
            return scale
